...
L.Util.extend(L.KML, {
  ...
  parsePlacemark: function (place, xml, style, options) {
    ...
    for (i = 0; i < el.length; i++) {
      for (j = 0; j < el[i].childNodes.length; j++) {
        if (
          !/Distance in km:.*?(?=\d,\d)/gm.exec(el[i].childNodes[j].nodeValue)
        ) {
        ...
        if (!/Notes: .*?(?=\s<br>)/gm.exec(el[i].childNodes[j].nodeValue)) {
          el[i].childNodes[j].nodeValue = el[i].childNodes[j].nodeValue.replace(
            /<br><br>Notes: /g,
            ""
          )
  ...
  parseLineString: function (line, xml, options) {
    ...
    return new L.Polyline(coords, { className: "kml-polyline", ...options })
  },
